...
for (const file of testFiles) {
  ...
  const objectLiteralPattern =
    /(ticketNFT\.connect\([^)]+\)\.mintTicket\()([^,]+),\s*([^,]+),\s*\{\s*eventId:\s*([^,]+),\s*price:\s*([^,]+),\s*validFrom:\s*([^,]+),\s*validUntil:\s*([^,]+),\s*isTransferable:\s*([^}]+)\s*\}\s*\)/g;

  content = content.replace(
    objectLiteralPattern,
    (match, prefix, to, tokenId, eventId, price, validFrom, validUntil, isTransferable) => {
      changes++;
  ...
  content = content.replace(
    /(const|let)\s+(\w+)\s*=\s*\{\s*eventId:\s*([^,]+),\s*price:\s*([^,]+),\s*validFrom:\s*([^,]+),\s*validUntil:\s*([^,]+),\s*isTransferable:\s*([^}]+)\s*\};([\s\S]*?)\.mintTicket\(([^,]+),\s*([^,]+),\s*\2\)/g,
    (
      match,
      varType,
      varName,
  ...
  content = content.replace(
    /(expect\(\s*ticketNFT\.connect\([^)]+\)\.mintTicket\()([^,]+),\s*([^,]+),\s*(\w+)(\)\s*\.[^;]+;)/g,
