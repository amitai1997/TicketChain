...
async function main() {
  ...
  if (!nameArg || !startArg || !endArg) {
    console.log('Usage: pnpm create-event --name="Event Name" --start="2025-06-01T19:00:00" --end="2025-06-01T23:00:00" [--uri="ipfs://metadata-uri"]');
    return;
  }
  ...
  const uri = uriArg ? uriArg.split('=')[1].replace(/"/g, '') : `ipfs://event-metadata/${encodeURIComponent(name)}`;
  ...
  console.log(`Creating event "${name}" with start: ${startDate.toISOString()} and end: ${endDate.toISOString()}`);
  ...
  if (receipt?.logs) {
    const eventCreatedLog = receipt.logs.find(
      (log: any) => log.topics[0] === ethers.id('EventCreated(uint256,address,uint256,uint256,string)')
    );
