...
function loadDeploymentInfo(network: string) {
  ...
  if (!fs.existsSync(deploymentPath)) {
    throw new Error(`Deployment file not found for network ${network}. Please deploy contracts first.`);
  }
...
async function main() {
  ...
  if (!eventArg || !quantityArg || !priceArg) {
    console.log('Usage: pnpm mint-tickets --event=1 --quantity=5 --price="0.1" [--transferable=true] [--recipient=0x...] [--validFromOffset=3600] [--validUntilOffset=7200]');
    return;
  }
  ...
  const eventRegistry = await ethers.getContractAt('EventRegistry', eventRegistryAddress) as EventRegistry;
  const eventTicket = await ethers.getContractAt('EventTicket', eventTicketAddress) as EventTicket;
  ...
  if (eventInfo) {
    ...
    try {
      ...
      console.log(`Found event on chain with start: ${new Date(eventData.startTime * 1000).toISOString()} and end: ${new Date(eventData.endTime * 1000).toISOString()}`);
    } catch (error) {
  ...
  console.log(`Ticket validity period: ${new Date(validFrom * 1000).toISOString()} to ${new Date(validUntil * 1000).toISOString()}`);
  ...
  if (recipientArg) {
    ...
    if (recipients.length < quantity) {
      console.log(`Warning: Only ${recipients.length} test accounts available, minting ${recipients.length} tickets instead of ${quantity}`);
    }
    ...
    console.log(`Ticket information saved to: ${path.join(ticketsDir, `tickets-${Date.now()}.json`)}`);
  }
}
